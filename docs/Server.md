...
_注意：通过该方法注册的 `preValidation` 钩子将在 Fastify 检测不到的路由中运行，即便你在路由函数中调用 [`reply.callNotFound`](https://github.com/fastify/docs-chinese/blob/master/docs/Reply.md#call-not-found) **也是如此**_。在这种情况下，只有 preHandler 会运行。
